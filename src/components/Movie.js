...
const Movie = (props) => {
  ...
  const movies = useSelector(newStore=>newStore.movieReducer.movies);
  const movie = movies.find(movie => movie.id === Number(id));
  function handleDelete(){;
  push('/movies'); console.log("silme oldu")
  return dispatch(deleteMovie(Number(id))) }
  function handleFavori(){;
  push('/movies'); console.log("favori ekledi")
  return dispatch(addFavorite(movie)) }
  ...
  return (
    <div className="bg-white rounded-md shadow flex-1">
      ...
      <div className="px-5 py-3 border-t border-zinc-200 flex justify-end gap-2">
        ...
        <button onClick={handleFavori} className="myButton bg-blue-600 hover:bg-blue-500 ">Favorilere ekle</button>
      </div>
    </div>
  );
}
